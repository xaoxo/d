"""Référentiels géographiques : départements et arrondissements de Paris/Lyon/Marseille."""
...
_PAR_NOM = {normalize(n): c for c, n in DEPARTEMENTS_NOMS.items()}
_PAR_NOM.update({"reunion": "974", "corse du sud": "2A", "haute corse": "2B", "belfort": "90"})


def departement_depuis_nom(texte: str | None) -> str | None:
    """« Bouches-du-Rhône », « bouches-du-rhone », « 13 » → « 13 »."""
    if not texte:
        return None
    t = normalize(texte)
    if t.upper() in DEPARTEMENTS_NOMS:
        return t.upper()
    if re.fullmatch(r"\d{1,3}", t):
        t2 = t.zfill(2)
        return t2 if t2 in DEPARTEMENTS_NOMS else None
    return _PAR_NOM.get(t)


_PLM = {"paris": ("751", 20), "lyon": ("6938", 9), "marseille": ("132", 16)}


def code_arrondissement(ville: str | None) -> str | None:
    """« Paris 15ème », « Lyon 3e », « Marseille 8e arrondissement » → code INSEE d'arrondissement."""
    m = re.match(r"^(paris|lyon|marseille)\s*(\d{1,2})\s*(?:e|eme|er|ieme)?\b", normalize(ville))
    if not m:
        return None
    prefixe, maxi = _PLM[m.group(1)]
    n = int(m.group(2))
    if not 1 <= n <= maxi:
        return None
    return f"{prefixe}{n:02d}" if m.group(1) != "lyon" else f"{prefixe}{n}"
